...
function FixLabIndexPage() {
	...
	useEffect(() => {
		api
			...
			.then((res) => {
				...
				navigate(`/fixlab/${latest.id}`, { replace: true });
			})
	...
	return (
		<div className="min-h-screen bg-[#eef2f8] flex flex-col items-center justify-center font-mono text-slate-500 gap-3">
			...
			<span className="text-xs tracking-wider uppercase">{error || "Loading Fix Lab…"}</span>
		</div>
	);
}

export default FixLabIndexPage;
